// Desk Radar — entry point / glue. SKELETON: TODOs mark what to implement.
...
#include <ESPmDNS.h>                // http://deskradar.local
...
static void handleRoot() {
    ...
    snprintf(buf, BUFSZ,
        ...
        "</style></head><body>"
        "<div class=hd><div class=dot></div><div><h1>Desk Radar</h1><p class=sub>Live ADS-B radar &middot; configuration</p></div></div>"
...
static void handleUpdatePage() {
    g_web.send(200, "text/html",
        "<!DOCTYPE html><html><head><meta charset=utf-8>"
        "<meta name=viewport content='width=device-width,initial-scale=1'>"
        "<title>Desk Radar - Update</title><style>"
        "body{background:radial-gradient(circle at 50% -10%,#0a1f15,#04100a 70%);color:#cdd6d1;"
...
void setup() {
    ...
    if (g_wm.autoConnect("deskradar-Setup"))
        Serial.println("[wifi] connected");
    else
        Serial.println("[wifi] config portal open - join 'deskradar-Setup' to set WiFi; UI stays live");
    ...
    // configuration web page (http://deskradar.local/)
    g_web.on("/", handleRoot);
