# Desk Radar — Live Flight Radar Desk Gadget
...
Desk Radar is a 3D-printed desk gadget that shows **live air traffic around your
...
- A built-in **web page** at `http://deskradar.local/` to set the **centre point
...
4. Open **`http://deskradar.local/`** on any device on the same network to fine-tune:
...
| Find the config page | See the **Stats** view footer (shows `deskradar.local` + IP) |
...
- **Can't reach `deskradar.local`** — use the **IP** shown on the Stats view instead.
